...
class Phone(Field):
    ...
    def value(self, value):
        if re.match(r"^(\+380\d{9}|0\d{9})$", value):
            ...
            print(
                "Incorrect phone number format, should be +380638108709 or 0638708106."
            )
...
class AddressBook(UserDict):
    ...
    def add_phone_to_contact(self, text):
        ...
        if len(name_input) >= 2 and len(phone_to_add) >= 1:
            for key, value in self.data.items():
                if key == name_input:
                    if phone_to_add not in list(
                        i.value.lower() for i in self.data[name_input].phones
                    ):
                        ...
                        if phone_to_add_new.value is not None:
                            ...
                            return print(
                                f"Phone{phone_to_add_new.value} is added for {name_input}"
                            )
                        else:
                            return
                    else:
                        return print(
                            f"This phone is already entered for this addressbook!"
                        )
    ...
    def add_email_to_contact(self, text):
        ...
        if len(name_input) >= 2 and len(email_to_add) >= 5:
            for key, value in self.data.items():
                if key == name_input:
                    if email_to_add not in list(
                        i.value.lower() for i in self.data[name_input].emails
                    ):
                        ...
                        return print(
                            f"This email is already entered for this addressbook!"
                        )
            return print(f"{name_input} was not found in addressbook")
        elif len(name_input) == 0:
            print("Enter name of contact!")
    ...
    def add_address_to_contact(self, text):
        ...
        if len(name_input) >= 2 and len(address_to_add) >= 5:
            for key, value in self.data.items():
                if key == name_input:
                    if address_to_add not in list(
                        i.value.lower() for i in self.data[name_input].addresses
                    ):
                        print(f"Address for {name_input} is added!")
                        return self.data[name_input].addresses.append(
                            Address(address_to_add)
                        )
                    else:
                        return print(
                            f"This address is already entered for this addressbook!"
                        )
            return print(f"{name_input} was not found in addressbook")
        elif len(name_input) == 0:
            print("Enter name of contact!")
    ...
    def birthday_in(self, days):
        ...
        for record in self.data.values():
            if record.birthday == "":
                ...
                edited_birthday = (
                    datetime.strptime(record.birthday.value, "%d.%m.%Y")
    ...
    def search_matches_in_addressbook(self, match):  # Шукаємо збіги в адресній книзі
        ...
        for name, contact in self.data.items():
            ...
            if (
                any(match in phone for phone in list_of_phones)
...
def main():
    ...
    all_commands = {
        ...
        "add birthday": lambda: contact_book.add_birthday_to_contact(
            text_after_command
        ),
        ...
        "find matches": lambda: contact_book.search_matches_in_addressbook(
            text_after_command
        ),
        # NoteBook commands
    ...
    while True:
        ...
        if command in closing_words:
            ...
            most_similar_command = difflib.get_close_matches(
                command_to_check_for_dif, commands, n=1
            )
            print(
                f"Invalid command. Print 'info' to see list of commands.\n"
                f"The most similar to command: '{command_to_check_for_dif}' is: '{most_similar_command[0]}'"
            ) if (len(most_similar_command) > 0) else (
